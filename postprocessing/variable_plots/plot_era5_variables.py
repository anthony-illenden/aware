...
def generate_file_paths(start_year, start_month, start_day, start_hour,
                        ...
                        base_path_vort):
    """
    Generate parallel lists of file paths for main and vorticity datasets
    for the specified date range (every 3 hours).
    """
    ...
    while current_date <= end_date:
        ...
        vort_paths.append(os.path.join(base_path_vort, filename))
        current_date += timedelta(hours=3)

    return main_paths, vort_paths


def load_dataset(main_path, vort_path, bounds=None):
    """
    Load and merge the main dataset file with the corresponding vorticity file.
    Both datasets are sliced to the specified lat/lon bounds and merged.
    """
    try:
        ds_main = xr.open_dataset(main_path).squeeze()
        ds_vort = xr.open_dataset(vort_path).squeeze()
        ...
        ds_main_sliced = ds_main.sel(**bounds)
        ds_vort_sliced = ds_vort.sel(**bounds)

        return xr.merge([ds_main_sliced, ds_vort_sliced])

    except Exception as e:
        print(f"Error loading datasets:\n  Main: {main_path}\n  Vorticity: {vort_path}\n  Error: {e}")
        return None
    
def has_twelve_hour_consecutive_trues(group, window_size):
    mask = group["overlap_qtr_deg"]
...
def load_cyclone_tracks(cyclone_path):
    ...
    df_track = df[df['overlap_qtr_deg'] == True]
    df_track['time'] = pd.to_datetime(df_track['time'])
    df_track["overlap_qtr_deg"] = df_track["overlap_qtr_deg"].astype(bool)
    ...
    storm_ids_to_keep = df_track.groupby("track_id").filter(lambda x: has_twelve_hour_consecutive_trues(x, window_size))["track_id"].unique()
...
def make_plots(ds_sliced, cyclone_path, save_path=None, directions=None):
    ...
    # Extract variables from sliced dataset
    mslp = ds_sliced.MSLP
    ivt = ds_sliced.IVT
    thetae_grad = ds_sliced.thetae_grad
    lat, lon = ds_sliced['latitude'], ds_sliced['longitude']
    
    # Optional: vorticity field if available
    vort = ds_sliced['vort_925']
    
    # Get current time from dataset as scalar Timestamp
...
def process_time_series(main_files,
                       ...
                       bounds=None):
    """
    Process multiple main+vorticity file pairs for a time series.
    """
    # Safety check: file list lengths must match
    if len(main_files) != len(vort_files):
        raise ValueError("Main files and vorticity files lists must have the same length.")
    ...
    for main_path, vort_path in zip(main_files, vort_files):
        print(f"Processing: {os.path.basename(main_path)} and {os.path.basename(vort_path)}")

        # Load merged dataset
        ds_sliced = load_dataset(main_path, vort_path, bounds)
...
if __name__ == "__main__":
    ...
    # File paths
    EVENTS_CSV_PATH = "/expanse/nfs/cw3e/csg101/aillenden/wy2015_dates.csv"
    BASE_PATH_MAIN = "/expanse/nfs/cw3e/csg101/aillenden/wy_era5/2015/"
    BASE_PATH_VORT = "/expanse/nfs/cw3e/csg101/aillenden/wy_era5/2015_extra/"
    CYCLONE_PATH = "/expanse/nfs/cw3e/csg101/aillenden/tempest_extremes/python_scripts/post_process/slp_triple_overlap.csv"
    OUTPUT_DIR = "/expanse/nfs/cw3e/csg101/aillenden/plots/tempest/center_tracks/"
    ...
    for _, row in df_events.iterrows():
        event_id = row.iloc[0]  # event number
        ...
        # Generate file paths
        main_files, vort_files = generate_file_paths(
